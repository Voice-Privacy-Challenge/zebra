...
class PriorLogOddsPlots(object):
    ...
    def __init__(self, classA_scores=None, classB_scores=None, normalize=False, plo=linspace(-10, 10, 201)):
        """
        ...
        self.Ptar = sigmoid(self.plo)
        self.Pnon = sigmoid(-self.plo)
        self.Pdcf = vstack([[self.Ptar, self.Pnon]]).T

        self.defDCF = minimum(self.Ptar, self.Pnon)
        self.defECE = (sigmoid(self.plo) * -log(sigmoid(self.plo)) + sigmoid(-self.plo) * -log(sigmoid(-self.plo))) / log(2)
    ...
    def add_legend_entry(self, entry):
        ...
        """
        self.__add_legend_enty_DCF(entry=entry)
        self.__add_legend_enty_ECE(entry=entry)

    def __add_legend_enty_DCF(self, entry):
        """
        adds legend entry to internal DCF plot only

        :param entry: profile label
        """
        self.legend_DCF.append(entry)

    def __add_legend_enty_ECE(self, entry):
        """
        adds legend entry to internal ECE plot only
    ...
    def set_system(self, classA_scores, classB_scores):
        ...
        cdet = self.Pdcf @ vstack((self.Pmiss, self.Pfa))
